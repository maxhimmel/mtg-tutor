 ...
 *
 * So the passage does not shorten and is never cut -- a caveat that only some
 * readers see is a caveat that stops being true. It gets a different PLACE, and
 * this is that place: a question mark, on the panel's own header rule.
 *
 * IT IS THE ONLY PLACE NOW. The page carried a switch for a while -- every
 * passage either a paragraph under its panel or a mark beside it -- which was
 * built to find out which read better and has done its job. A setting is a
 * question the page asks its reader, and this one was really a question the page
 * was asking us; leaving it up would mean every future passage has to be written
 * to work in two positions, and half of the screen's readers would be looking at
 * a layout nobody was designing for.
 *
 * WHICH SECTIONS USE IT IS STILL A JUDGEMENT, and not every passage does. A mark
 * earns its place where the prose COSTS something -- four lines out of the rail's
 * scarce column, a caption under an instrument a reader has already learnt. The
 * short version laid across the page keeps its sentence in plain sight, because
 * there it sits in space that was empty anyway. So each section decides, and this
 * only draws the mark.
 *
 * IT IS NOT A HOVER-ONLY CONTROL, which is the trap the mark itself has to
 * avoid. It is a real button: it takes focus, it opens on focus as well as on
 * hover, and it names its subject out loud rather than saying "more info". A

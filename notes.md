...
1. **The colour-pair half shipped; the DECK-TYPE half is what is left.**
   ...
   **The gate is worth reading before touching this.** Best-minus-worst is a
   ...
   wide even when nothing is there -- a flat two-sigma gate passes 18.7% of pure
   noise at four decks and 59.8% at ten, which turned a reported bank of 1,149
   questions into a real one of 362. Trap #13, caught by asking for the null
   rather than by anything looking wrong. The threshold is now per deck count
   (`RANGE_CRITICAL` in core, regenerated by
   `pnpm diagnose-archetype-quiz --calibrate`) and the setting is a
   false-positive rate.
...
23. **The widest gap among k noisy numbers is wide when nothing is there, and a
    ...
    really 362, and the false ones were concentrated on the cards played in the
    ...
    becomes a false-positive RATE. `RANGE_CRITICAL` in
    `core/drills/archetypes.ts` carries the table;
    `pnpm diagnose-archetype-quiz --calibrate` regenerates it. Its k=2 row must
    come out at 1.96, because with nothing to maximise over the range test IS
    the ordinary two-sided z-test -- a table that misses that is wrong
    elsewhere too.
